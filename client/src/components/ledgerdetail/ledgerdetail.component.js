  ...
  useEffect(() => {
    async function getRecords() {
      ...
      try {
        ...
        const filterdata = records.filter(record => record.trn_type.trim() !== 'OPBAL')
        const exchCodes = [...new Set(records.map(record => record.exchcode))];
        const trnTypes = [...new Set(records.map(record => record.trn_type))];

        const cli_cod = [...new Set(records.map(record => record.cli_cod))];
        const location = [...new Set(records.map(record => record.locnid))];
...
//     // Sort by year, then month, then day from `lgr_date`
//     filtered.sort((a, b) => {
//       if (a.cli_cod > b.cli_cod) return -1;
//       if (a.cli_cod < b.cli_cod) return 1;
      
//       // Since lgr_date is in 'yyyy-mm-dd' format, we can directly compare the strings
//   if (a.lgr_date < b.lgr_date) return -1;
//   if (a.lgr_date > b.lgr_date) return 1;
...
//     // Apply the same sorting logic for downloading
...
//         index + 1,
//         record.ks_locnid, record.locnid, record.eslucc, record.cli_cod, record.clname,
//         record.exchcode, formatDate(record.lgr_date), record.vouch_no, record.narr,
//         record.trn_type, record.cheq_no, record.dr_amt, record.cr_amt, record.cumlative,
//         record.cum_bal, record.intamt, record.docno
//       ]),
//     ];
...
// }
  ...
  //   //   return monthB - monthA || dayA - dayB;
  //   // });

  // // Sort records by year, then month, then day from lgr_date
  // filtered.sort((a, b) => {
  //   const [yearA, monthA, dayA] = a.lgr_date.split('-').map(Number);
  //   const [yearB, monthB, dayB] = b.lgr_date.split('-').map(Number);

  //   return yearB - yearA || monthB - monthA || dayB - dayA;
  // });

  // // Sort by cli_cod (ascending) and then exchcode (ascending)
  // filtered.sort((a, b) => {
  //   if (a.cli_cod !== b.cli_cod) {
  //     return a.cli_cod.localeCompare(b.cli_cod);
  //   }
  //   return a.exchcode.localeCompare(b.exchcode);
  // });
