...
const prioService = require("../services/prio.service");

router.get("/", async (req, res) => {
  try {
    var people = await prioService.getAll();
    res.json(people);
  } catch (error) {
    console.log(error);
...
router.post("/", async (req, res) => {
  try {
    var createdPerson = await prioService.createPerson(req.body);
...
router.get("/:id", async (req, res) => {
  ...
  try {
    const person = await prioService.findPersonById(id);
    if (!person) {
...
router.patch("/:id", async (req, res) => {
  const id = req.params.id;
  try {
    const existingPerson = await prioService.findPersonById(id);
    if (!existingPerson) {
      return res.status(404).json({ statusCode: 404, error: `Person with id ${id} does not exist` });
    }
    const updatedPerson = await prioService.updatePerson(req.body);
...
router.delete("/:id", async (req, res) => {
  try {
    var exisitingPerson = await prioService.findPersonById(req.params.id);
    if (!exisitingPerson) {
      return res
        .status(404)
    ...
    await prioService.deletePerson(req.params.id);
    return res.json({
      statusCode: 200,
