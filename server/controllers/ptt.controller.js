...
const pttService = require("../services/ptt.service");

router.get("/", async (req, res) => {
  try {
    var people = await pttService.getAll();
    res.json(people);
  } catch (error) {
    console.log(error);
...
router.post("/", async (req, res) => {
  try {
    var createdPerson = await pttService.createPerson(req.body);
...
router.get("/:id", async (req, res) => {
  ...
  try {
    const person = await pttService.findPersonById(id);
    if (!person) {
...
router.patch("/:id", async (req, res) => {
  const id = req.params.id;
  try {
    const existingPerson = await pttService.findPersonById(id);
    if (!existingPerson) {
      return res.status(404).json({ statusCode: 404, error: `Person with id ${id} does not exist` });
    }
    const updatedPerson = await pttService.updatePerson(req.body);
...
router.delete("/:id", async (req, res) => {
  try {
    var exisitingPerson = await pttService.findPersonById(req.params.id);
    if (!exisitingPerson) {
      return res
        .status(404)
    ...
    await pttService.deletePerson(req.params.id);
    return res.json({
      statusCode: 200,
