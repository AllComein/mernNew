...
const dealslipService = require("../services/dealslips.service");
...
router.post("/", async (req, res) => {
  try {
    var createdPerson = await dealslipService.createPerson(req.body);
...
router.get("/:id", async (req, res) => {
  ...
  try {
    const person = await dealslipService.findPersonById(id);
    if (!person) {
...
router.patch("/:id", async (req, res) => {
  const id = req.params.id;
  try {
    const existingPerson = await dealslipService.findPersonById(id);
    if (!existingPerson) {
      return res.status(404).json({ statusCode: 404, error: `Person with id ${id} does not exist` });
    }
    const updatedPerson = await dealslipService.updatePerson(req.body);
...
router.delete("/:id", async (req, res) => {
  try {
    var exisitingPerson = await dealslipService.findPersonById(req.params.id);
    if (!exisitingPerson) {
      return res
        .status(404)
    ...
    await dealslipService.deletePerson(req.params.id);
    return res.json({
      statusCode: 200,
