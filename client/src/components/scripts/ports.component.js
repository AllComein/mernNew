   ...
   useEffect(() => {
    async function fetchAllData() {
      ...
      try {
        ...
        const filteredFoRecords = foRecords.filter(record => viewableUsers.includes(record.locnid));
        const filteredComRecords = comRecords.filter(record => viewableUsers.includes(record.locnid));

        setAllRecords({
          porteq: filteredEqRecords,
          portfo: filteredFoRecords,
          portcom: filteredComRecords,
        });
      } catch (error) {
...
//         const currentusername = authService.getCurrentUser()
//         const filteredEqRecords = eqRecords.filter(record => record.locnid === currentusername.username);
//         const filteredFoRecords = foRecords.filter(record => record.locnid === currentusername.username);
//         const filteredComRecords = comRecords.filter(record => record.locnid === currentusername.username);
