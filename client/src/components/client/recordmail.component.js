...
const contentRef = useRef(null);
  ...
  useEffect(() => {
    async function getdatas() {
      try {
        setLoading(true);
        const response = await fetch(`http://183.182.84.228:4005/pfdiff/`);
        if (!response.ok) {
          const message = `An error occurred: ${response.statusText}`;
          window.alert(message);
          return;
...
function filteredRecords() {
  ...
  function generateAndDownloadView() {
    ...
    const data = [
         ["S.No.","ES UCC",	"KS UCC",	"Client Name", "Pan No." ,	"KS KRA Status",	"KS PTT Status",	"KS KRA/UCC Sts",	"KS KRA/PTT Remarks",	"KS KRA/PTT e-mail Sent",	"KS PTT e-mail Recd",	"KS KRA Remark"
          /* Add other headers as needed */],
          ...filtered.map((record, index) => {
            const parsedStatusDate = record.date ? new Date(record.date.split('-').reverse().join('-')) : '';
            return[
            index + 1,record.ucc,	record.kslucc,	record.clname, record.panno ,	record.ks_krasts,	record.ks_pttsts,	record.ks_kusts,	record.ks_ltstrm,	record.ks_emsent,	record.ks_emrecd,	record.ks_eslrm
...
async function handleMails1() {
  const filters = [applyFilter1, applyFilter2, applyFilter3 ];
  const sheetNames = ['clsr-tally', 'clsr-pf-sts-not-exist', 'clsr-pf-diff']; // Add your sheet names here
  const wb = XLSX.utils.book_new();
  const Values = ['EXCLUSIVE DP CLOSER DONE, PORTFOLIO STATUS NO DIFF. , NOT SENT TO KOTAK ', 'EXCLUSIVE DP CLOSER DONE, PORTFOIL STATUS NOT FOUND, NOT SENT TO KOTAK ', 'EXCLUSIVE DP CLOSER DONE, PORTFOIL IN DIFF, NOT SENT TO KOTAK '];
  let lastIndex = 0;
  let sheetCounts = [];
  ...
  for (let i = 0; i < filters.length; i++) {
    ...
    ...filtered.map((record, index) => {
      lastIndex = index + 1;
      return [lastIndex , record.priority ,	record.brcd ,	record.ucc ,	record.kslucc ,	record.clname ,	record.ks_krasts ,	record.ks_pttsts ,	record.closer_trf ,	record.ksl_date ,	record.kslstatus ,	record.es_pfreco ,	record.brname ,
      ]}),
    ];
    const ws = XLSX.utils.aoa_to_sheet(wsData);
    XLSX.utils.book_append_sheet(wb, ws, sheetNames[i]);
    sheetCounts.push({sheetName: Values[i], count: lastIndex});
  }

    // Create a new sheet for row counts
    const rowCountData = [["Sheet Name", "Row Count"], ...sheetCounts.map(sheet => [sheet.sheetName, sheet.count])];
    const rowCountWs = XLSX.utils.aoa_to_sheet(rowCountData);
    XLSX.utils.book_append_sheet(wb, rowCountWs, "Row Counts");

  const currentDate = new Date();
  const formattedDate = `${currentDate.getDate()}-${currentDate.getMonth() + 1}-${currentDate.getFullYear()}`;
  const filename = `Exclusive Closer Done , Kotak PortFolio Pending_${formattedDate}.xlsx`;
  const wbout = XLSX.write(wb, { bookType: 'xlsx', type: 'array' });
  const xlsxFile = new Blob([wbout], { type: 'application/octet-stream' });
  const file = new File([xlsxFile], filename, { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
  ...
  Please Find attachment for Closer transfer Portfolio Pending, 
  Please status revert to all email id's.
  
  ${sheetCounts.map(sheet => `Total No. of Records in  ${sheet.sheetName}::   ${sheet.count}`).join('\n')}
  
  Regards,
  Exclusive Securities Cons. LLP , Indore`;
  const subject = `Exclusive Closer Done , Kotak PortFolio Pending`;

  setMailerState((prevState) => ({
    ...prevState,
    name: 'record1.clname',
    recipients: 'dmat@exclusivegroup.co.in , rajneeshu@exclusivegroup.co.in , bhatter469a@gmail.com , manish.eslmail@gmail.com , samaiya16@yahoo.co.in , umeshd@exclusivegroup.co.in , YOGESH_EXCLUSIVE@YAHOO.COM',
    message: message,
    subject:subject,
    file: file,
  }));
...
async function handleMails2() {
  const filters = [ applyFilter4 , applyFilter5 , applyFilter6];
  const sheetNames = [ 'KSUCC-OPN-KRAPNDG' , 'ucc-without-barcode--sts-pndg' , 'ucc-with-barcode-sts-pndg']; // Add your sheet names here
  const wb = XLSX.utils.book_new();
  const Values = ['Exclusive - Kotak Ucc Open - KRA Pending', 'Exclusive - Esl Client Acitve & Kotak Ucc Status is Still Pending for Barcode Generation', 'Exclusive - Esl Client Acitve & Kotak Ucc Status is Still Pending & Barcode Already Generated'];
  let lastIndex = 0;
  ...
  for (let i = 0; i < filters.length; i++) {
    ...
    const wsData = [
      ...
      ...filtered.map((record, index) => {
        lastIndex = index + 1;
        return [lastIndex, record.priority ,	record.barcode ,	record.brcd ,	record.ucc ,	record.kslucc ,	record.clname , 	record.ks_krasts ,	record.ks_pttsts ,	record.ks_krarem ,	record.ks_emsent ,	record.ks_emrecd ,	record.ks_rm ,	record.ks_krptrem ,	record.sts1_3i	,record.sts2_3i	,record.ks_othsts	,record.mult_emmo	,record.ks_allrem ,	record.catg ,	record.panno ,	record.eckyc_no ,	record.ks_dpid ,	record.ks_dpboid ,	record.ks_clsts ,	record.es_clsts ,	record.bo_clsts ,	record.ks_emtocl ,	record.ks_esign ,	record.ks_essts ,	record.ks_emailid ,	record.es_email ,	record.is_mtch_em ,	record.ks_mobile ,	record.es_mobile ,	record.is_mtch_mo ,	record.es_netbkg ,	record.ks_lstrdt ,	record.es_lstrdt ,	record.closer_trf ,	record.ksl_date ,	record.kslstatus ,	record.es_mtfpos ,	record.es_opnpos ,	record.es_margin ,	record.es_ledbal ,	record.es_dpledg ,	record.es_dphldg ,	record.es_ccol ,	record.es_nccol ,	record.es_pledval ,	record.es_corpact ,	record.es_pfreco ,	record.brname ,	record.family_grp ,	record.run_date];
      }),
    ];
    const ws = XLSX.utils.aoa_to_sheet(wsData);
    XLSX.utils.book_append_sheet(wb, ws, sheetNames[i]);
  ...
  const currentDate = new Date();
  const formattedDate = `${currentDate.getDate()}-${currentDate.getMonth() + 1}-${currentDate.getFullYear()}`;
  const filename = `Exclusive Active But Not Tradable in Kotak_${formattedDate}.xlsx`;
  const wbout = XLSX.write(wb, { bookType: 'xlsx', type: 'array' });
  const xlsxFile = new Blob([wbout], { type: 'application/octet-stream' });
  const file = new File([xlsxFile], filename, { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
  ...
  Please Find attachment for Pending Closer Transfer from ESL To Kotak, 
  Please status revert to all email id's.
  ...
  Regards,
  Exclusive Securities Cons. LLP , Indore`;
  const subject = `Exclusive Active But Not Tradable in Kotak`;

  setMailerState((prevState) => ({
    ...prevState,
    name: 'record1.clname',
    recipients: 'koushlendra.upadhyay@kotak.com , prakashp@exclusivegroup.co.in , rams@exclusivegroup.co.in , bhatter469a@gmail.com , manish.eslmail@gmail.com , samaiya16@yahoo.co.in , umeshd@exclusivegroup.co.in , YOGESH_EXCLUSIVE@YAHOO.COM',
    message: message,
    subject:subject,
    file: file,
  }));
...
<td>{record.editdt}</td>
           ...
           <th onClick={() => setShowHoverTable1(!showHoverTable1)} className="animated-th1">Portfolio Reconsilation
           {showHoverTable1 && (
              <div className="hover-table">
                <button  onClick={() => setShowHoverTable1(!showHoverTable1)}>close</button>
...
<th>STATUS</th>
                  ...
                  {datas &&
  datas
    .filter((record) => record.UCC === viewRecordDetails.ucc) // Filter details based on the kslucc of the viewed record
    .map((record, index) => (
      <tr key={index}>
        <td>{record.PF_UNI}</td>
<td>{record.UCC}</td>
...
{!previewMode && (
  <>
        ...
        <button onClick={handleSubmit}>Mail</button>
        <button onClick={clearSearchAndSelection}>Clear</button>
        <button onClick={mailexcel1}>Download Exclusive Closer Done , Kotak PortFolio Pending</button>
        <button onClick={mailexcel2}>Download Exclusive Active But Not Tradable in Kotak</button><br/>
      ...
      <div className="dropdown">
        <div className="dropbtn">date Difference</div>
        <div className="dropdown-content">
          <label>
            <input
              type="radio"
...
{previewMode && (
      ...
      {loading && (
        ...
        &nbsp;&nbsp;&nbsp;
    <button onClick={handleMails1}>Exclusive Closer Done , Kotak PortFolio Pending</button>
    &nbsp;&nbsp;&nbsp;
    <button onClick={handleMails2}>Exclusive Active But Not Tradable in Kotak</button>
    &nbsp;&nbsp;&nbsp;
