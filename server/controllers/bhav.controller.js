...
const bhavService = require("../services/bhav.service");

router.get("/", async (req, res) => {
  try {
    var people = await bhavService.getAll();
    res.json(people);
  } catch (error) {
    console.log(error);
...
router.post("/", async (req, res) => {
  try {
    var createdPerson = await bhavService.createPerson(req.body);
...
router.get("/:id", async (req, res) => {
  ...
  try {
    const person = await bhavService.findPersonById(id);
    if (!person) {
...
router.patch("/:id", async (req, res) => {
  const id = req.params.id;
  try {
    const existingPerson = await bhavService.findPersonById(id);
    if (!existingPerson) {
      return res.status(404).json({ statusCode: 404, error: `Person with id ${id} does not exist` });
    }
    const updatedPerson = await bhavService.updatePerson(req.body);
...
router.delete("/:id", async (req, res) => {
  try {
    var exisitingPerson = await bhavService.findPersonById(req.params.id);
    if (!exisitingPerson) {
      return res
        .status(404)
    ...
    await bhavService.deletePerson(req.params.id);
    return res.json({
      statusCode: 200,
