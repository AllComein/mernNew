...
export default function DormancyLetters() {
  ...
  useEffect(() => {
    async function getRecords() {
      ...
      const filteredRecords = records.filter(record => filteredLetterData && record.ks_krarem && record.ks_krarem.includes('Dormant') || record.nod_dormnt >= 0 && record.nod_dormnt <= 7
        );


        // const filteredRecords = records.filter(record =>
        //   currentUser.username === record.es_locnid &&
        //   record.ks_krarem && record.ks_krarem.includes('Dormant')
        // );
