    ...
    useEffect(() => {
      async function fetchAllData() {
        ...
        const [eqRecords, fuRecords, opRecords, comRecords] = await Promise.all([
          ...
          const filteredEqRecords = eqRecords.filter(record => viewableUsers.includes(record.locnid));
          const filteredFuRecords = fuRecords.filter(record => viewableUsers.includes(record.locnid));
          const filteredOpRecords = opRecords.filter(record => viewableUsers.includes(record.locnid));
          const filteredComRecords = comRecords.filter(record => viewableUsers.includes(record.locnid));

          const allowedTypes = ['FUTIDX', 'FUTSTK'];
        const filterfurecords = filteredFuRecords.filter(record => allowedTypes.includes(record.instrument_type));
        const notallowedTypes = ['OPTIDX', 'OPTSTK'];
        const filteroprecords = filteredOpRecords.filter(record => notallowedTypes.includes(record.instrument_type));
  
          setAllRecords({ porteq: filteredEqRecords, portfu: filterfurecords, portop: filteroprecords, portcom: filteredComRecords });
        } catch (error) {
  ...
  const handleSubmit = () => {
    ...
    setTimeout(() => {
      try {
        ...
        // Apply filters
        if (clientCodeList.length > 0 || fromDate || toDate) {
...
// export default function Brokrages() {
//     const [loading, setLoading] = useState(false);
//     const [processing, setProcessing] = useState(false);
//     const [allRecords, setAllRecords] = useState({ porteq: [], portfu: [], portop: [], portcom: [] });
//     const [filteredRecords, setFilteredRecords] = useState([]);
...
//             // Apply filters
//             if (clientCodeList.length > 0 || locnidList.length > 0 || fromDate || toDate) {
//           records = records.filter(record => {
//             const clientCode = (record.client_code || "").trim().toUpperCase();
...
//     const data = [
//         ["Sno.","Client Code","client name","Client location","locnID","Squp Brokrage","Dilevery Brokrage","Squp Volume","Dilevery Volume",`Squp Brokrage(${percentage.eq.squp})`,`Dilevery Brokrage (${percentage.eq.dlbrok})`,"Diff Squp","Diff Dilevery"],
//         ...searchedRecords.map((record, index) => [
//           index + 1, 
//           record.client_code,
//           record.cl_name,
...
//           parseFloat(record.broksqup.toFixed(2)),
//           parseFloat(record.brokdlv.toFixed(2)),
...
// //     const data = [
// //       ["Sno.", "Client Code", "Client Name", "Client Location", "LocnID", "Squp Brokerage", "Delivery Brokerage", "Squp Volume", "Delivery Volume", `Squp Brokerage (${percentage[segment]?.dlbrok || 0}%)`, `Delivery Brokerage (${percentage[segment]?.dlbrok || 0}%)`, "Diff Squp", "Diff Delivery"],
// //       ...records.map((record, index) => [
// //         index + 1,
// //         record.client_code || "",
// //         record.cl_name || "",
...
//     const uniquelocn = Array.from(new Set(filteredRecords.map(record => record.Clientlocation)));
//     const uniqueUccs = Array.from(new Set(filteredRecords.map(record => record.client_code)));
