...
export default class BoardPtts extends Component {
  constructor(props) {
    super(props);
...
//   };
  ...
  //     //   (record) =>
  //     //     currentUser.username === record.locncd
  //     // );
  ...
  getLetterData = async () => {
    try {
      ...
      if (currentUserDetails) {
        ...
        const filteredLetterData = letterData.filter(record =>
          viewableUsers.includes(record.es_locnid)
        );
...
render() {
    ...
    <br/><br/>
            ...
            <p>Data is not available in PTT.</p>
          )}
        </div>
      </div>
    );
