...
router.get("/", async (req, res) => {
  try {
    var people = await diffService.getAll();
    res.json(people);
  } catch (error) {
    console.log(error);
...
router.post("/", async (req, res) => {
  try {
    var createdPerson = await diffService.createPerson(req.body);
...
router.get("/:id", async (req, res) => {
  ...
  try {
    const person = await diffService.findPersonById(id);
    if (!person) {
...
router.patch("/:id", async (req, res) => {
  const id = req.params.id;
  try {
    const existingPerson = await diffService.findPersonById(id);
    if (!existingPerson) {
      return res.status(404).json({ statusCode: 404, error: `Person with id ${id} does not exist` });
    }
    const updatedPerson = await diffService.updatePerson(req.body);
...
router.delete("/:id", async (req, res) => {
  try {
    var exisitingPerson = await diffService.findPersonById(req.params.id);
    if (!exisitingPerson) {
      return res
        .status(404)
    ...
    await diffService.deletePerson(req.params.id);
    return res.json({
      statusCode: 200,
